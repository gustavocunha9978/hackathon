    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        artigo => 
          ...
          artigo.autores.some(autor => autor.nome.toLowerCase().includes(query))
      );
    }
            ...
            <Card key={artigo.id} className="p-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">{artigo.resumo.substring(0, 120)}...</p>
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <FileText className="h-4 w-4" />
                    <span>{artigo.area_tematica}</span>
                    <span className="mx-1">•</span>
                    <Users className="h-4 w-4" />
                    <span>{artigo.autores.map(autor => autor.nome).join(', ')}</span>
                    <span className="mx-1">•</span>
                    <span>Submetido em {formatDate(artigo.versao_atual.data_cadastro)}</span>
                  </div>
                </div>
