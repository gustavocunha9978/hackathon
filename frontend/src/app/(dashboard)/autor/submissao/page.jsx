...
export default function SubmissaoPage() {
    ...
    const fetchEventos = async () => {
      try {
        ...
        if (!formData.evento_id && eventosAtivos.length > 0) {
          setFormData(prev => ({ ...prev, evento_id: String(eventosAtivos[0].id) }));
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validações básicas
    if (!formData.titulo || !formData.resumo || !formData.area_tematica || !formData.evento_id || !formData.arquivo) {
    ...
    try {
      ...
      const novoArtigo = await createArtigo({
        ...
        autores: [
          ...
          ...coautores.filter(coautor => coautor.nome && coautor.email)
            .map((coautor, index) => ({ 
              id: 1000 + index, 
              nome: coautor.nome 
            }))
