      ...
      try {
        ...
        if (!artigoData) {
          ...
          if (artigoData.autores && artigoData.autores.length > 1) {
            setCoautores(artigoData.autores.slice(1).map(autor => ({
              nome: autor.nome,
              email: autor.email || ''
            })));
          }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <h2 className="mt-4 text-xl font-semibold">Artigo não pode ser editado</h2>
          <p className="mt-2 text-muted-foreground">
            Este artigo não pode ser editado no momento devido ao seu status atual.
          </p>
          <Button 
            variant="outline" 
            className="mt-6" 
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="arquivo">Arquivo PDF do Artigo (Nova Versão) *</Label>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {artigo?.versao_atual && (
                  <span className="block mt-1">
                    Versão atual: {artigo.versao_atual.versao} (enviada em {formatDate(artigo.versao_atual.data_cadastro)})
                  </span>
                )}
              </p>
            </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {coautores.map((coautor, index) => (
                <div key={index} className="space-y-4 p-4 border rounded-md bg-secondary/20">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        value={coautor.nome}
                        onChange={(e) => handleCoautorChange(index, 'nome', e.target.value)}
                        disabled={isSaving}
                      />
                    </div>
                    <div className="space-y-2">
        ...
        {artigo.status === 'revisao' && (
          <Card className="mt-6 bg-yellow-50 border-yellow-200">
            <CardHeader>
              <CardTitle className="text-yellow-800">Comentários do Avaliador</CardTitle>
              <CardDescription className="text-yellow-700">
                Leia os comentários dos avaliadores e faça as correções necessárias.
              </CardDescription>
            </CardHeader>
